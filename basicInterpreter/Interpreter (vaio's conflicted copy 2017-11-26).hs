...
interpNext = do
  ...
  let forLine = fromJust $ lookup "for" newTable
...
interpFor = do
  ...
  let finalTable = addToTable "for" (show $ (lineNum interp) + 1) table1
...
interpIf = do
  ...
  let s = source interp !! lineNum interp
  let bool = getBool s  
  let evaled = interpBool $ initReplaceSymbols bool (table interp)
  let jumpLine = convertLineNum (show (getAction s)) (source interp) 0
...
interpBool b =
  if isInfixOf "<>" b then evalBool (/=) "<>" b
  else if isInfixOf ">="  b then evalBool (>=) ">=" b
  else if isInfixOf "<=" b then evalBool (<=) "<=" b
  else if isInfixOf "<" b then evalBool (<) "<" b
  else if isInfixOf ">" b then evalBool (>) ">" b
  else if isInfixOf "=" b then evalBool (==) "=" b
...
evalBool op str b =
  op (read first :: Float) (read second :: Float) where
  first = head $ splitOn str b
  second = head (tail $ splitOn str b)
...
interpPrint newLine = do
  ...
  if newLine then liftIO $ print (trimSpaces out)
    else liftIO $ putStr (trimSpaces out)
...
trimSpaces [] = []
trimSpaces (c:cs) =
  if c == ' ' && (head cs == ' ') then trimSpaces cs
  else c : trimSpaces cs
...
parseFun = do
  s <- string "log" +++ string "rnd" +++ string "abs" +++ string "sqrt"
  return s
